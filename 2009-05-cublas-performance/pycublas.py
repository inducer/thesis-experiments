...
class CUBLASMatrix(object):
  ...
  def __init__(self, X, dtype=numpy.float32, mem_align=True):
    ...
    if isinstance(X, (numpy.matrix, numpy.ndarray)):
...
#  B = numpy.mat( numpy.array( numpy.random.random_sample((8191,257)), numpy.float32) )
  #A = numpy.array( numpy.random.random_sample((256,65536)), numpy.float32)
  #B = numpy.array( numpy.random.random_sample((65536,256)), numpy.float32)
...
#  print A*B
  ...
  print numpy.nonzero(numpy.isnan(C1))
  print (numpy.isnan(C1)).any()
  print (numpy.isnan(C2)).any()
